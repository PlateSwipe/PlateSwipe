...
class MealDBRecipeRepository(private val client: OkHttpClient) :  RecipeRepository{
  ...
  private val mealDBUrl = "https://www.themealdb.com/api/json/v1/1/"
    ...
    private fun parseMealDBJsontoRecipe(json: JSONObject): List<Recipe>{
        ...
        for (i in 0 until listOfRecipes.length()){
            ...
            for (j in 1..20){
                ...
                val measurement = meal.getString("strMeasure$j")
                if (ingredient.isNotEmpty() && measurement.isNotEmpty()){
                    ingredientsAndMeasurements.add(Pair(ingredient, measurement))
                }
            }
            parsedListOfRecipes.add(Recipe(idMeal, strMeal, strCategory, strArea, strInstructions, strMealThumbUrl, ingredientsAndMeasurements))
        }
     ...
     */
    override fun random(nbOfElements: Long, onSuccess: (List<Recipe>) -> Unit, onFailure: (Exception) -> Unit){
    ...
    private fun getOneRandomRecipe(onSuccess:(List<Recipe>)->Unit, onFailure: (Exception) -> Unit){

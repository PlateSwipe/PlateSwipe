...
class FirestoreRecipesRepositoryTest {
  ...
  fun documentToRecipe_validDocument() {
    ...
    `when`(document.get(FIRESTORE_RECIPE_INGREDIENTS)).thenReturn(listOf("Ingredient1", "Ingredient2"))
    `when`(document.get(FIRESTORE_RECIPE_MEASUREMENTS)).thenReturn(listOf("Measurement1", "Measurement2"))
    ...
    assertEquals(listOf("Ingredient1" to "Measurement1", "Ingredient2" to "Measurement2"), recipe?.ingredientsAndMeasurements)

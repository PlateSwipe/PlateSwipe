...
class PlateSwipeDropdownMenuTest {
    ...
    fun testDropdownAllComponentsAreDisplayed() {
        ...
        composeTestRule.onAllNodesWithTag(DROPDOWN_ITEM, useUnmergedTree = true).assertCountEquals(exampleList.size)
    ...
    fun testPlateSwipeSelectsCorrectly() {
        ...
        composeTestRule.setContent { PlateSwipeDropdownMenu(exampleList, onSelected = {string , index ->
            wasSelected = string
            selectedIndex = index
        }) }
        ...
        composeTestRule.onNodeWithText(exampleList[1], useUnmergedTree = true).assertIsDisplayed().performClick()
    ...
    fun testPlateSwipeCorrectlyUpdatesTitleAfterSelection() {
        ...
        composeTestRule.setContent { PlateSwipeDropdownMenu(exampleList)
        noneSelectedString = stringResource(R.string.no_item_selected)
        }

        composeTestRule.onNodeWithTag(DROPDOWN_TITLE, useUnmergedTree = true).assertIsDisplayed().assertTextEquals(noneSelectedString)

        composeTestRule.onNodeWithTag(DROPDOWN).assertIsDisplayed().performClick()

        composeTestRule.waitForIdle()

        composeTestRule.onNodeWithText(exampleList[0], useUnmergedTree = true).assertIsDisplayed().performClick()

        composeTestRule.waitForIdle()

        composeTestRule.onNodeWithTag(DROPDOWN_TITLE, useUnmergedTree = true).assertIsDisplayed().assertTextEquals(exampleList[0])
    }
}